/* leaflet と leaflet.offline をもとに実装 */
...
import {LatLngBounds, toLatLngBounds as latLngBounds} from 'leaflet/src/geo/LatLngBounds';
...
export var GridLayer = Layer.extend({
	...
	_update: async function (center) {
		...
		if (queue.length !== 0) {
			...
			for (i = 0; i < queue.length; i++) {
				await this._addTile(queue[i], fragment);
			}
	...
	_addTile: async function (coords, container) {
		var tilePos = this._getTilePos(coords),
		    key = this._tileCoordsToKey(coords);

		var tile = await this.createTile(this._wrapCoords(coords), Util.bind(this._tileReady, this, coords));
