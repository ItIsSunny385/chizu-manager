import {GridLayer} from 'leaflet/src/layer/tile/GridLayer';
...
export var OfflineTileLayer = GridLayer.extend({
	...
	createTile: async function (coords, done) {
		...
		*/
		...
		tile.src = await this.getTileUrl(coords);
	...
	getTileUrl: async function (coords) {
		...
		var tmp = Util.template(this._url, Util.extend(data, this.options));
		const response = await (async () => { return "https://cyberjapandata.gsi.go.jp/xyz/std/1/1/1.png"})();
		return response;
	},
...
export function offlineTileLayer(url, options) {
